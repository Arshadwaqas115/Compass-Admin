...
const Chart = ({ setPath, docId, type }) => {
  ...
  const steps = [
    ...
    {
      name: 'Accomodation',
      component: <Accomodation data={data?.accomodation} customer={data?.mainDetails} type={type} />,
    },
  ...
  const filteredSteps =
    type === 'Agent' ? steps.filter((step) => step.name === 'Accomodation' || step.name === 'Transport') : steps;
    ...
    <div>
      <div className="pb-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {type === 'Agent' && <div className="text-xl">{data?.name}</div>}
          {type === 'User' && <div className="text-xl">{data?.mainDetails?.guestName}</div>}
        </div>
        <div>
          <button
